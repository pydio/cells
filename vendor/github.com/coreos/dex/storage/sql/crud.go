...
func (c *conn) GarbageCollect(now time.Time) (result storage.GCResult, err error) {
	r, err := c.Exec(`delete from dex_auth_request where expiry < $1`, now)
	if err != nil {
		return result, fmt.Errorf("gc dex_auth_request: %v", err)
	}
	if n, err := r.RowsAffected(); err == nil {
		result.AuthRequests = n
	}

	r, err = c.Exec(`delete from dex_auth_code where expiry < $1`, now)
	if err != nil {
		return result, fmt.Errorf("gc dex_auth_code: %v", err)
	}
	if n, err := r.RowsAffected(); err == nil {
		result.AuthCodes = n
	}
...
func getAuthRequest(q querier, id string) (a storage.AuthRequest, err error) {
	...
	err = q.QueryRow(`
		...
		from dex_auth_request where id = $1;
	`, id).Scan(
	...
	fmt.Printf("Authrequest Get: %v", a.PClaims)
	return a, nil
}

func (c *conn) CreateAuthCode(a storage.AuthCode) error {
	_, err := c.Exec(`
		insert into dex_auth_code (
...
func getRefresh(q querier, id string) (storage.RefreshToken, error) {
	return scanRefresh(q.QueryRow(`
		select
			...
			connector_id, connector_data,
			token, created_at, last_used
		from dex_refresh_token where id = $1;
	`, id))
}
...
func scanRefresh(s scanner) (r storage.RefreshToken, err error) {
	...
	if err != nil {
		...
		return r, fmt.Errorf("scan dex_refresh_token: %v", err)
	}
...
func (c *conn) UpdateClient(id string, updater func(old storage.Client) (storage.Client, error)) error {
	return c.ExecTx(func(tx *trans) error {
		...
		if err != nil {
			return fmt.Errorf("update dex_client: %v", err)
		}
		return nil
	})
}
...
func (c *conn) CreateClient(cli storage.Client) error {
	...
	if err != nil {
		...
		return fmt.Errorf("insert dex_client: %v", err)
	}
	return nil
}
...
func (c *conn) CreatePassword(p storage.Password) error {
	...
	if err != nil {
		...
		return fmt.Errorf("insert dex_password: %v", err)
	}
	return nil
}
...
func (c *conn) UpdatePassword(email string, updater func(p storage.Password) (storage.Password, error)) error {
	return c.ExecTx(func(tx *trans) error {
		...
		if err != nil {
			return fmt.Errorf("update dex_password: %v", err)
		}
		return nil
	})
}
...
func getPassword(q querier, email string) (p storage.Password, err error) {
	return scanPassword(q.QueryRow(`
		select
			email, hash, username, user_id
		from dex_password where email = $1;
...
func (c *conn) CreateConnector(connector storage.Connector) error {
	...
	if err != nil {
		...
		return fmt.Errorf("insert dex_connector: %v", err)
	}
	return nil
}
...
func (c *conn) DeleteAuthRequest(id string) error { return c.delete("dex_auth_request", "id", id) }
func (c *conn) DeleteAuthCode(id string) error    { return c.delete("dex_auth_code", "id", id) }
func (c *conn) DeleteClient(id string) error      { return c.delete("dex_client", "id", id) }
func (c *conn) DeleteRefresh(id string) error     { return c.delete("dex_refresh_token", "id", id) }
func (c *conn) DeletePassword(email string) error {
	return c.delete("dex_password", "email", strings.ToLower(email))
}
