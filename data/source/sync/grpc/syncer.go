...
func (s *Syncer) Config(ctx context.Context) (*object.DataSource, error) {
	...
	if syncConfig == nil {
		return nil, errors.WithMessage(errors.StatusNotFound, "cannot find config for "+s.DsName)
	}
...
func (s *Syncer) watchDisconnection() {
	...
	for w := range watchOnce {
		if m, ok := w.(*model.EndpointStatus); ok && m.WatchConnection == model.WatchDisconnected {
			...
			if e := s.InitSync(s.GlobalCtx); e != nil {
