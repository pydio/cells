...
func (e *Handler) recomputeNodePlainSize(ctx context.Context, node *tree.Node, dsName string) (int64, error) {
	nodeEncryptionClient := encryption.NewNodeKeyManagerClient(grpc.ResolveConn(ctx, common.ServiceEncKeyGRPC, grpc.WithSilentNotFound()))
	psResp, psErr := nodeEncryptionClient.GetNodePlainSize(ctx, &encryption.GetNodePlainSizeRequest{
		NodeId: node.Uuid,
		UserId: "ds:" + dsName,
	})
	if psErr != nil {
	...
	r, rErr := pool.GetTreeClient().ReadNode(ctx, &tree.ReadNodeRequest{Node: &tree.Node{Uuid: node.Uuid}})
	if rErr != nil {
		return 0, rErr
	}
	update := r.GetNode()
	update.Size = plainSize
	_, uerr := pool.GetTreeClientWrite().CreateNode(ctx, &tree.CreateNodeRequest{Node: update, UpdateIfExists: true})
	if uerr != nil {
		return 0, uerr
	}
