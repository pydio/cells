...
type URLOpener interface {
	OpenURL(ctx context.Context, u *url.URL) (Conn, error)
}
...
func (mux *URLMux) Register(scheme string, opener URLOpener) {
	mux.schemes.Register("server", "Server", scheme, opener)
}

// OpenConn calls OpenURL with the URL parsed from urlstr.
func (mux *URLMux) OpenConn(ctx context.Context, urlstr string) (Conn, error) {
...
// OpenConn opens the Connection identified by the URL given.
// See the URLOpener documentation in driver subpackages for
// details on supported URL formats, and https://gocloud.dev/concepts/urls
// for more information.
func OpenConn(ctx context.Context, urlstr string) (Conn, error) {
	return defaultURLMux.OpenConn(ctx, urlstr)
}
