...
func (a *AclQuotaFilter) FindParentWorkspaces(ctx context.Context, workspace *idm.Workspace) (parentWorkspaces []*idm.Workspace, parentContext context.Context, err error) {
	...
	for _, root := range workspace.RootUUIDs {
		...
		if er != nil {
			log.Logger(ctx).Error("AncestorsList for rootNode", zap.Any("r", root), zap.Int("ancestors length", len(ancestors)), zap.Any("ownerWsRoots", ownerWsRoots))
			err = er
			return
		}
		...
		log.Logger(ctx).Debug("Workspace Parents?", zap.Int("parents length", len(parentWorkspaces)))
