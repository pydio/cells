...
func (b *EncryptedBlock) Write(writer io.Writer) (int, error) {
	...
	dataLine := fmt.Sprintf("Data hash 	   : %s\n", base64.StdEncoding.EncodeToString(crypto.Md5(b.Payload)))
...
type AESGCMEncryptionMaterials struct {
	...
	blockChan         chan EncryptedBlockHeader

	mode int
}

// NewRangeAESGCMMaterials creates an encryption materials that use AES GCM
func NewAESGCMMaterials(key []byte, info *encryption.NodeInfo, blockChan chan EncryptedBlockHeader) *AESGCMEncryptionMaterials {
...
func (m *AESGCMEncryptionMaterials) legacyDecryptRead(b []byte) (int, error) {
	...
	for totalPlainBytesRead < l {
		...
		if encryptedBufferCursor != 0 && (encryptedBufferCursor == int(m.encryptedBlockSize) || m.eof) {
			...
			opened, err := crypto.Open(m.encryptionKey, nonce, encryptedBufferPart)
			if err != nil {
				return 0, err
			}
			encryptedBufferCursor = 0
	...
	for totalRead < l {
		...
		if !m.eof {
			...
			if count > 0 {
				...
				data, err := crypto.SealWithNonce(m.encryptionKey, nonce, buff)
				if err != nil {
					return 0, err
				}
...
func (m *AESGCMEncryptionMaterials) decryptRead(b []byte) (int, error) {
	totalRead := 0
	l := len(b)
	leftToRead := int(m.plainRangeLimit - m.totalProcessedServed)
	...
	for totalRead < l {
		...
		if !m.eof {
			...
			if count > 0 {
				data, err := crypto.Open(m.encryptionKey, b.Header.Nonce, b.Payload)
				if err != nil {
					return 0, err
				}
