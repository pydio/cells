	...
	for extUserId, extUser := range extUsers {
		...
		if apiUser, ok := apiUsers[extUserId]; ok {
			...
			if len(ExtUser.Roles) > 0 {
				log.Logger(context.Background()).Info("External User Roles", zap.Int("roles length", len(ExtUser.Roles)))
			}
...
func (m *Merger) LoadAndDiffRoles(ctx context.Context, params map[string]interface{}) (*models.RoleDiff, error) {
	...
	log.Logger(ctx).Debug("Source Roles", zap.Any("params", params), zap.Int("source length", len(src)))
	target, e := m.Target.ListRoles(ctx, m.Target, params)
	if e != nil {
		return nil, e
	}
	log.Logger(ctx).Debug("Target Roles", zap.Any("params", params), zap.Int("target length", len(target)))
	m.Diff(src, target, diff)
