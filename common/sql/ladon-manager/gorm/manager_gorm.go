...
// GORMManager is a gorm implementation for Manager to store policies persistently.
...
func NewManager(db *gorm.DB) *GORMManager {
	m := &GORMManager{
		db:       db,
		instance: func() *gorm.DB { return db.Session(&gorm.Session{SkipDefaultTransaction: true}) },
	}
	...
	err := m.instance().AutoMigrate(&Resource{}, &Action{}, &Subject{}, &Policy{})
	fmt.Println(err)
...
func (s *GORMManager) Create(policy ladon.Policy) (err error) {
	tx := s.instance().Clauses(clause.OnConflict{DoNothing: true}).Create((&Policy{}).From(policy))
	if tx.Error != nil {
		return tx.Error
	}
...
func (s *GORMManager) Update(policy ladon.Policy) error {
	tx := s.instance().Updates((&Policy{}).From(policy))
	if tx.Error != nil {
		return tx.Error
	}
...
func (s *GORMManager) FindRequestCandidates(r *ladon.Request) (ladon.Policies, error) {
	var policies []Policy
...
func (s *GORMManager) FindPoliciesForResource(resource string) (ladon.Policies, error) {
...
func (s *GORMManager) FindPoliciesForSubject(subject string) (ladon.Policies, error) {
...
func (s *GORMManager) GetAll(limit, offset int64) (ladon.Policies, error) {
	var policies []Policy
	tx := s.instance().
		Preload("Actions").Preload("Resources").Preload("Subjects").
...
func (s *GORMManager) Get(id string) (ladon.Policy, error) {
	var policy Policy
	tx := s.instance().Preload("Actions").Preload("Resources").Preload("Subjects").Where(&Policy{ID: id}).Find(&policy)
...
func (s *GORMManager) Delete(id string) error {
	tx := s.instance().Where(&Policy{ID: id}).Delete(&Policy{})
