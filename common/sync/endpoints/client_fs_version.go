...
import (
	...
	"github.com/pydio/cells/common/sync"
...
func (c *FSClientVersion) Walk(walknFc sync.WalkNodesFunc, pathes ...string) (err error) {
...
func (c *FSClientVersion) isIgnoredFile(eventPath string) bool {
	return sync.IsIgnoredFile(eventPath) || strings.Contains(eventPath, ".pydio.versions") || strings.Contains(eventPath, ".pydio.writes")
}

// Watches for all fs events on an input path.
func (c *FSClientVersion) Watch(recursivePath string) (*sync.WatchObject, error) {

	eventChan := make(chan sync.EventInfo)
	errorChan := make(chan error)
	doneChan := make(chan bool)
	// Make the channel buffered to ensure no event is dropped. Notify will drop
	// an event if the receiver is not able to keep up the sending pace.
	in, out := sync.PipeChan(1000)

	var fsEvents []notify.Event
	fsEvents = append(fsEvents, sync.EventTypeAll...)
	...
	go func() {
		for event := range out {
			...
			eventInfo, eventError := sync.notifyEventToEventInfo(c, event)
			if eventError != nil {
				log.Println("Sending  event error for " + c.RootPath)
				errorChan <- eventError
			} else if eventInfo != (sync.EventInfo{}) {
...
func NewFSClientVersion(rootPath string) *FSClientVersion {
	return &FSClientVersion{
		...
		FS:       afero.NewBasePathFs(afero.NewOsFs(), sync.CanonicalPath(rootPath)),
	}
}
