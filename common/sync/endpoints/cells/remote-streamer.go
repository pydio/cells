...
func (c *Remote) BulkLoadNodes(ctx context.Context, nodes map[string]string) (map[string]interface{}, error) {
	...
	sendCtx, can := context.WithTimeout(newCtx, 10*time.Minute)
	defer can()
	...
	for path, nodePath := range nodes {
		...
		if err != nil || !resp.Success {
			results[path] = errors.WithStack(errors.NodeNotFound)
		} else {
			out := resp.Node
...
func (c *Remote) FlushSession(ctx context.Context, sessionUuid string) error {
	...
	sendCtx, can := context.WithTimeout(c.getContext(ctx), 10*time.Minute)
	defer can()
