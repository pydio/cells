...
const validationMessages = {
    ...
    'meta.user.validation.maxLength':
        'Must contain no more than {limit} characters.',
...
describe('mapErrors', () => {
    ...
    it('maps required errors when schemaPath includes #required', () => {
        ...
        const errors = [
            createError(
                'required',
                '/usermeta-text',
                '#/properties/usermeta-text/required',
                'must be present',
                { missingProperty: 'usermeta-text' },
            ),
        ];
    ...
    it('maps instancePath errors', () => {
        const errors = [
            createError(
                'minLength',
                '/usermeta-text',
                '#/properties/usermeta-text/minLength',
    ...
    it('handles nested instance paths', () => {
        const errors = [
            createError(
                'minLength',
                '/nested/property',
                '#/properties/nested/property/minLength',
                'too short',
                {},
            ),
        ];
    ...
    it('aggregates multiple errors', () => {
        const errors = [
            createError('required', '', '#/required', 'missing property', {
                missingProperty: 'field1',
            }),
            createError(
                'maxLength',
                '/field2',
                '#/properties/field2/maxLength',
                'too long',
                {},
            ),
        ];
    ...
    it('handles missingProperty containing slash', () => {
        const errors = [
            createError('required', '', '#/required', 'missing property', {
                missingProperty: 'field/with/slash',
            }),
        ];
        const result = mapErrors(errors);
        expect(result).toEqual({
...
describe('buildValidator', () => {
    ...
    it('returns invalid state with errors for data violating schema', () => {
        ...
        expect(result.errors['usermeta-text']).toBe(
...
describe('buildValidator with conditional AJV defaults', () => {
    ...
    it('applies defaults when applyDefaults: true', () => {
        const validate = buildValidator(schemaWithDefaults, {
            applyDefaults: true,
        });
...
describe('Multi-value metadata validation', () => {
    it('validates array fields with valid enum values', () => {
        const validate = buildValidator(multiValSchema, {
            applyDefaults: false,
        });
    ...
    it('validates maxLength constraint on text field', () => {
        ...
        const formState = new Map([
            ...
            [
                'usermeta-long-text',
                'This is a very long text that definitely exceeds the one hundred character limit set in the schema for this field and should fail',
            ],
            ['usermeta-number', 42],
        ]);
    ...
    it('formatSpecialCasesForValidation preserves array values', () => {
        ...
        const result = formatSpecialCasesForValidation(
            formState,
            multiValSchema,
        );
