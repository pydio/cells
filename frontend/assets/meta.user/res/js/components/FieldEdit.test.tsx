...
describe('FieldEdit Component', () => {
    ...
    describe('validation and error handling', () => {
        ...
        it('prevents save-intent when validation errors exist in tag_cloud field', async () => {
            const setShouldSave = vi.fn();
            const setFormState = vi.fn();
            const context = createContext({
                state: {
            ...
            // Add a tag to trigger onCommitChange
            const input = screen.getByRole('textbox') as HTMLInputElement;
            fireEvent.change(input, { target: { value: 'document' } });
            fireEvent.keyPress(input, {
                key: ',',
            ...
            // setFormState should be called to update the value
            expect(setFormState).toHaveBeenCalled();

            // setShouldSave should NOT be called with true because errors exist
            expect(setShouldSave).toHaveBeenCalledWith(false);
        });

        it('allows save-intent when no validation errors exist in tag_cloud field', async () => {
            const setShouldSave = vi.fn();
            const setFormState = vi.fn();
            const context = createContext({
                state: {
            ...
            // setShouldSave should be called with true because no errors exist
            expect(setShouldSave).toHaveBeenCalledWith(true);
        });
    });
    ...
    describe('shouldHideLabel behavior', () => {
        it('hides label when shouldHideLabel is true', () => {
            const context = createContext({
                state: {
                    formState: new Map([['testField', '']]),
                    saving: false,
            ...
            // Label should not be visible when shouldHideLabel is true
            expect(screen.queryByText('Test Label')).not.toBeInTheDocument();
        });

        it('shows label when shouldHideLabel is false or undefined', () => {
            const context = createContext({
                state: {
                    formState: new Map([['testField', '']]),
                    saving: false,
            ...
            // Label should be visible when shouldHideLabel is false
            expect(screen.getByText('Test Label')).toBeInTheDocument();
        });

        it('shows label by default when shouldHideLabel is not provided', () => {
            const context = createContext({
                state: {
                    formState: new Map([['testField', '']]),
                    saving: false,
            ...
            // Label should be visible by default when shouldHideLabel is not provided
