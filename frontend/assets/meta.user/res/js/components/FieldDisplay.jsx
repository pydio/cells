...
const NumberDisplay = ({ value, format }) => {
    ...
    return (
        <span>{`${getNumberPrefix(format)}${value || ''}${getNumberSuffix(format)}`}</span>
    );
};
...
export const FieldDisplay = ({
    ...
    switch (type) {
        ...
        case 'date':
            displayValue = (
                <DateTimeField node={node} column={column} type={type} />
            );
            break;
        case 'integer':
            displayValue = (
                <NumberDisplay value={value} format={(data || {}).format} />
            );
            break;
        case 'boolean':
    ...
    return (
        ...
        >
            ...
            <div className="infoPanelValue" onClick={onValueClick}>
                {displayValue}
            </div>
        </div>
    );
