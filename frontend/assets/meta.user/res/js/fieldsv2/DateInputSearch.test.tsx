...
vi.mock('@mantine/dates', () => ({
    DatePickerInput: ({ value, onChange, leftSection, ...props }: any) => (
        <div data-testid="date-picker-input">
            <output data-testid="current-date">{value?.toISOString() || 'null'}</output>
            <div data-testid="left-section">{leftSection}</div>
            <button
                data-testid="date-input-change"
                onClick={() => onChange(value ? null : new Date('2024-02-10T10:00:00.000Z'))}
            >
                toggle
            </button>
            <button
...
describe('DateInputSearch', () => {
    ...
    describe('integration flow', () => {
        ...
        it('should preserve value through edit cycle with modifier', () => {
            ...
            expect(screen.getByTestId('current-date')).toContainHTML('2024-02-10')
    ...
    describe('popover and focus behavior', () => {
        it('should pass requestToggleClose to popover when provided', () => {
            const onChange = vi.fn()
        ...
        it('should set autoFocus when requestToggleClose is provided', () => {
            const onChange = vi.fn()
            const requestToggleClose = vi.fn()
        ...
        it('should not set onBlur when requestToggleClose is not provided', () => {
            const onChange = vi.fn()
            ...
            // Component should work fine without requestToggleClose
            expect(screen.getByTestId('date-picker-input')).toBeInTheDocument()
        })
    })
})
