...
function createPydioMock() {
    return {
        requireLib: vi.fn((lib) => {
            if (lib === 'hoc') {
                return {
                    ModernAutoComplete: ({ searchText, hintText, onUpdateInput, onNewRequest, onClose }) =>
                        React.createElement('div', { 'data-testid': 'autocomplete-container' }, [
...
vi.mock('material-ui', () => ({
    ...
    AutoComplete: ({ searchText, hintText, onUpdateInput, onNewRequest, onClose }) =>
        React.createElement('input', {
            'data-testid': 'tags-input',
            value: searchText || '',
            placeholder: hintText,
...
describe('TagsCloud - Edit Mode', () => {
    ...
    it('adds a new tag when input is submitted', () => {
        ...
        render(React.createElement(TagsCloud, { ...defaultProps, updateValue }));
        const input = screen.getByTestId('tags-input');
...
describe('TagsCloud - Value Prop Mode', () => {
    ...
    it('updates when value prop changes', () => {
        const { rerender } = render(React.createElement(TagsCloud, defaultProps));
        let chips = screen.getAllByTestId('chip');
        expect(chips).toHaveLength(3);

        rerender(React.createElement(TagsCloud, { ...defaultProps, value: "newTag1,newTag2" }));
