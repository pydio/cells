...
const { ThemedModernStyles, ModernAutoComplete, colorsFromString } =
...
class TagsCloud extends React.Component {
    ...
    suggestionLoader(callback) {
        const { skipAutoSuggest, fieldname, column, autoSuggestTagName } =
            this.props;
        if (skipAutoSuggest) {
            callback([]);
            return;
        ...
        MetaClient.getInstance()
            .listTags(autoSuggestTagName || fieldname || column.name)
            .then((tags) => {
                this.setState({ loading: this.state.loading - 1 });
                callback(tags);
            });
    ...
    handleRequestDelete(tag) {
        ...
        let tags = parseTagsValue(this.state.tags);
    ...
    handleNewRequest() {
        ...
        let newTags = parseTagsValue(tags);
    ...
    render() {
        ...
        knownTags = parseTagsValue(tags)
            .map((tag) => LangUtils.trim(tag, ' '))
            .filter((tag) => !!tag);
