...
import { parseValueForValidation, parseErrors } from '../utils/jsonSchema';
...
export const useValidation = ({ configs, namespaceJsonSchema }) => {
    ...
    useEffect(() => {
        ...
        const propertyValidator = Object.entries(
            namespaceJsonSchema.properties,
        ).reduce((acc, [ns, cfg]) => {
        ...
        jsonSchemaRef.current = {
            userMetaJsonSchemaValidator,
            propertyValidator,
        };
    }, [configs, jsonSchemaRef.current, namespaceJsonSchema]);

    const globalValidate = useCallback(
        (values) => {
    ...
    const localValidate = useCallback(
        (values, namespaces = []) => {
            ...
            namespaces.forEach((namespace) => {
                ...
                const value = parseValueForValidation(
                    validator.schema,
                    values[namespace],
                );
                if (!validator(value)) {
