...
let StandardLayout = createReactClass({
    ...
    render(){
        ...
        if(searchTools.searchLoading) {
            ...
            let msg;
            if(hasMore) {
                msg = pydio.MessageHash['searchengine.results.withMore'].replace('%1', searchTools.limit)
            } else {
                msg = pydio.MessageHash['searchengine.results.foundN'].replace('%1', searchTools.resultsCount)
            }
            loadMoreButton = (
                <span style={{opacity: 0.7,  color: 'white',  fontSize: 13,  float: 'left', marginTop: 13}}>{msg}</span>
            )
            if(hasMore) {
                loadMoreButton = (
                    <Fragment>
