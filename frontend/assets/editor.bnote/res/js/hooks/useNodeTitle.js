...
const renameFolder = (node, newName) => {
    ...
    NodeServiceApiFactory(pydio)
        .then((api) => {
            const fullPath =
                pydio.user.getActiveRepositoryObject().getSlug() +
                node.getPath();
            ...
            pydio.UI.displayMessage(
                'SUCCESS',
                t('hook.renaming').replace('%s', newName),
            );
...
export const useNodeTitle = ({ node }) => {
    ...
    const abstractMeta = Pydio.getInstance()
        .getPluginConfigs('editor.bnote')
        .get('BNOTE_ABSTRACT_META');
...
export const getPageTitleFromNode = (node) => {
    ...
    if (node.getMetadata().has('ws_root')) {
        title =
            Pydio.getInstance().user.getActiveRepositoryObject().getLabel() ||
            title;
