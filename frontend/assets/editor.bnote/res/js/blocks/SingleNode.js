    ...
    return (
        <div
            style={{
                borderRadius: 8,
                border: border
                    ? '1px solid var(--md-sys-color-mimefont-background)'
                    : '',
                overflow: 'hidden',
        ...
        >
            ...
            {rich && (
                ...
                >
                    ...
                    {menuGroups && <BlockMenu groups={menuGroups} />}
                </div>
            )}
        </div>
    );
...
const SingleNode = (props) => {
    ...
    const isCurrentFolder = useMemo(() => {
        ...
        return (
            LangUtils.trim(contextNodePath, '/') ===
            LangUtils.trim(targetPath, '/')
        );
    }, [node, dataModel]);

    const metaPageName = Pydio.getInstance()
    ...
    const displayMenu = useSingleNodeDisplay({
        node,
        crtValue,
        ...
        <span style={{ display: 'inline-flex' }}>
            ...
            {!error && (
                <span style={{ flex: 1, padding: '0 8px' }}>
                    {node ? node.getLabel() : PathUtils.getBasename(path)}
                </span>
            )}
        </span>
    );
        ...
        >
            ...
            {node && node.isLeaf() && (
                ...
                <HoverCard
                    ...
                    onOpen={selectOnHover}
                    onClose={() => setOpen(false)}
                >
                    <HoverCard.Target>{inlineLabel}</HoverCard.Target>
                    <HoverCard.Dropdown>
                        <Preview border={true} node={node} rich={false} />
                    </HoverCard.Dropdown>
                </HoverCard>
            )}
            {isPage && !error && (
