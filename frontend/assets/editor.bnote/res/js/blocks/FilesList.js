...
const { FilePreview, useRichMetaLine, useRichMetaActions } =
...
export const FilesList = muiThemeable()(({
    pydio,
    dataModel,
    contextNode,
    nodePath,
    ...
    const sortMenuGroup = useMemo(() => {
        const sortMenuItems = Object.keys(columns)
            .filter((k) => columns[k].sortType)
            .map((k) => {
                const col = columns[k];
                return {
                    value: { ...col, attribute: k },
                    title: col.label,
                    icon: MdSort,
                };
            });
    ...
    const actions = useSingleNodeActions({
        node: resolvedNode,
        presetNodeActions,
    });
    const menuGroups = resolvedNode
        ? [actions, displayMenuGroup, sortMenuGroup]
        : [displayMenuGroup, sortMenuGroup];

    const entryRenderIcon = useCallback(
        (node) => {
            const lightBackground = display === 'grid' || display === 'masonry';
            const hasThumbnail =
                !!node.getMetadata().get('thumbnails') ||
                !!node.getMetadata().get('ImagePreview');
            const processing = !!node.getMetadata().get('Processing');
            const uploading =
                node.getMetadata().get('local:UploadStatus') === 'loading';
            const uploadprogress = node
                .getMetadata()
                .get('local:UploadProgress');
            return (
                <FilePreview
    ...
    const entryRenderActions = useRichMetaActions({
        pydio,
        dataModel,
        displayMode: display,
    });
    ...
    return (
        ...
        >
            <Paper
                ...
                withBorder={true}
                p={'md'}
                style={{ lineHeight: '1.3em', width: '100%' }}
                {...hoverProps}
            >
                <div style={{ display: 'flex' }}>
                    ...
                    >
                        <span
                            style={{ marginRight: 6 }}
                            className={'mdi mdi-folder-open-outline'}
                        />
                        ️
                        <span style={{ marginRight: 6, flex: 1 }}>
                            {(resolvedNode && resolvedNode.getLabel()) ||
                                nodePath ||
                                t('files-list.toc')}
                            <span
                ...
                {contextNode && (
                    <ModernSimpleList
                        ...
                        entryRenderSecondLine={
                            display === 'list' ? entryRenderSecondLine : null
                        }
                        entryRenderActions={
                            display !== 'detail' && !nodePath
                                ? entryRenderActions
                                : null
                        }
