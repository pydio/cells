...
class MetaNamespace extends React.Component {
    ...
    save() {
        ...
        if (!hasSchema && namespace.JsonDefinition) {
            ...
            const needsEntity =
                ...
                type === 'auto_complete' ||
                type === 'choice';
            const entityItems = def.data?.entityItems || [];
            // if (needsEntity && entityItems.length === 0) {
            //     pydio.UI.displayMessage('ERROR', 'No entity items to save');
            //     return;
            // }
            ...
            Metadata.createEntity(
                `Entity for ${namespace.Namespace}`,
                namespace.Description,
                namespace.Policies || [],
            ).then((r) => {
                const updated = {
                    ...def,
                    entity: { entity_id: r.Uuid },
                };
                namespace.JsonDefinition = JSON.stringify(updated);
                this.setState({ namespace });
                const savePromise = doSave();
                if (entityItems.length > 0) {
                    return Promise.all([
                        savePromise,
                        Metadata.putEntityValues(
                            r.Uuid,
                            entityItems,
                            namespace.Policies || [],
                        ),
    ...
    getEntityItems() {
        ...
        try {
            const parsed = JSON.parse(namespace.JsonDefinition || '{}');
            const entityItems =
                parsed && parsed.data && parsed.data.entityItems;
