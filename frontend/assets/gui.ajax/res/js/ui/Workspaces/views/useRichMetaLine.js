...
const useRichMetaLine = ({
    pydio,
    columns,
    searchResults = false,
    searchScope = 'all',
    disableGotoLink = false,
    requestCloseOnGoto = undefined,
}) => {
    return useCallback(
        (node) => {
            ...
            if (metaData.has('pending_operation')) {
                ...
                return (
                    <span
                        className={'metadata_chunk metadata_chunk_description'}
                    >
                        {metaData.get('local:entryDescription')}
                    </span>
                );
            }
            ...
            if (metaData.has('ajxp_modiftime')) {
                let mDate = moment(
                    parseFloat(metaData.get('ajxp_modiftime')) * 1000,
                );
            ...
            if (
                ...
                pieces.push(
                    <span
                        ...
                        key="etag_temp_description"
                        title={
                            Pydio.getMessages()[
                                'ajax_gui.file.temporary-etag.tooltip'
                            ]
                        }
                        className="metadata_chunk metadata_chunk_description"
                    >
                        ...
                        {
                            Pydio.getMessages()[
                                'ajax_gui.file.temporary-etag.flag'
                            ]
                        }
                    </span>,
                );
            ...
            Object.keys(columns).forEach((s) => {
                ...
                if (
                    ...
                    s === 'text' ||
                    s === 'ajxp_modiftime' ||
                    columnDef.inlineHide
                ) {
                    return;
                } else if (s === 'ajxp_dirname' && metaData.get('filename')) {
                    let dirName = PathUtils.getDirname(
                ...
                let cellClass =
                    'metadata_chunk metadata_chunk_' +
                ...
                const cell = (
                    <span key={s} className={cellClass}>
                        {sep}
        ...
        [searchResults, searchScope, pydio, columns, disableGotoLink],
    );
};
