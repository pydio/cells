...
class Workspace extends Observable{
    ...
    getMetaSourceLabel(metaKey){
        let node = XMLUtils.XPathSelectSingleNode(this.xmlData, 'admin_data/metasources/meta[@id="'+metaKey+'"]/@label');
        if (node) {
    ...
    getMetaSourceDescription(metaKey){
        let node = XMLUtils.XPathSelectSingleNode(this.xmlData, 'admin_data/metasources/meta[@id="'+metaKey+'"]/@description');
        if(node){
            return node.nodeValue;
        }else{
            return metaKey;
