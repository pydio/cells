    ...
    function QuickLinks(props, context) {
        ...
        this.state = {
            ...
            links: [{
                ...
                label: getMessage('2', 'ajxp_conf'),
                description: getMessage('139', 'ajxp_conf')
            }, {
                path: '/data/repositories',
                iconClass: 'mdi mdi-network',
                label: getMessage('3', 'ajxp_conf'),
                description: getMessage('138', 'ajxp_conf')
            }]
        };
    }
    ...
    _createClass(QuickLinks, [{
        ...
        value: function render() {
            ...
            var links = this.state.links.map((function (l) {
                ...
                if (this.state.edit) {
                    label = React.createElement(
                        'span',
                        null,
                        React.createElement('span', { className: 'icon-remove button-icon' }),
                        ' ',
                        l.label
                    );
            ...
            if (this.state.edit) {
                ...
                menuItems = menuItems.concat(AdminComponents.NavigationHelper.buildNavigationItems(pydio, rootNode, muiTheme.palette));
                dropDown = React.createElement(
                    'div',
                    null,
                    React.createElement(
                        DropDownMenu,
                        { onChange: this.menuClicked.bind(this), value: '-1' },
                        menuItems
                    )
                );
            } else {
                dropDown = React.createElement(
                    'h4',
                    { style: { padding: '10px 16px 0', fontWeight: 400 } },
            ...
            return React.createElement(
                ...
                React.createElement(IconButton, { onTouchTap: this.toggleEdit.bind(this), iconClassName: this.state.edit ? 'icon-ok' : 'mdi mdi-pencil', secondary: this.state.edit })
            );
        }
    }]);
