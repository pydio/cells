...
func (h *Handler) DeleteEntityValues(ctx context.Context, req *idm.GetMetaEntityValuesRequest) (*idm.DeleteEntityValuesResponse, error) {
	dao, err := manager.Resolve[meta.DAO](ctx)
	if err != nil {
		return nil, err
	}
	...
	resp, err := evDAO.DeleteEntityValues(ctx, req.EntityUuid)
	if err != nil {
		return nil, err
	}
