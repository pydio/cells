...
func (a *Handler) WriteAllowed(ctx context.Context, acl *idm.ACL) error {
	...
	} else if acl.NodeID != "" && acl.Action != nil && (acl.Action.Name == permissions.ACL_READ.Name || acl.Action.Name == permissions.ACL_WRITE.Name) {
...
func (a *Handler) CheckNode(ctx context.Context, nodeID string, action *idm.ACLAction) error {
	...
	var check bool
	if action.GetName() == permissions.ACL_READ.GetName() {
	...
	} else if action.GetName() == permissions.ACL_WRITE.GetName() {
