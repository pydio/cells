...
func (h *Handler) PruneTokens(ctx context.Context, in *pauth.PruneTokensRequest) (*pauth.PruneTokensResponse, error) {
	...
	if err = storage.FlushInactiveAccessTokens(ctx, time.Now().Add(-1*time.Hour), 10000, 200); err != nil {
		return nil, err
	}

	if err = storage.FlushInactiveLoginConsentRequests(ctx, time.Now().Add(-1*time.Hour), 10000, 200); err != nil {
		return nil, err
	}

	// Flush inactive refresh tokens older than 3 months
	if err = storage.FlushInactiveRefreshTokens(ctx, time.Now().Add(-90*24*time.Hour), 10000, 200); err != nil {
		return nil, err
	}
