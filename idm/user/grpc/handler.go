...
func NewHandler(ctx context.Context, svc service.Service, dao user.DAO) idm.UserServiceServer {
	return &Handler{
		ctx:     ctx,
		Service: svc,
		dao:     dao,
...
func (h *Handler) CreateUser(ctx context.Context, req *idm.CreateUserRequest) (*idm.CreateUserResponse, error) {
	...
	for _, g := range createdNodes {
		if g.GetNode().GetUuid() != out.Uuid && g.GetNode().GetType() == tree.NodeType_COLLECTION {
			...
			if err := h.dao.AddPolicies(false, g.GetNode().GetUuid(), defaultPolicies); err != nil {
				return nil, err
			}
		}
	}
