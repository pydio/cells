...
func (c *OfficeHoursCondition) Fulfills(value interface{}, _ *ladon.Request) bool {
	...
	if parseErr != nil {
		log.Logger(runtime.CoreBackground()).Error("cannot parse match conditions. reference layout is "+officeHoursLayout, zap.String("Matches predicate", c.Matches), zap.Error(parseErr))
		return false
	}
