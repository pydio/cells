...
var ConfigureCmd = &cobra.Command{
 ...
 As recommended by databases documentations, make sure not to leave the 'max_connections' parameter 
 to its default value (151) while going live in production.
   
 If you are on a desktop machine, you can choose 'browser install' at first prompt, or directly call:
   $ ` + os.Args[0] + ` install --bind default
 ...
 In case where default port is busy, you can choose another via the 'bind' flag, for instance:
   $ ` + os.Args[0] + ` install --bind 0.0.0.0:12345
 or 	
   $ ` + os.Args[0] + ` install --bind <your server IP or FQDN>:12345
 ...
 If you are more a shell person, you can perform the configuration process directly using this CLI (using the 
 '--cli' flag or by choosing so at first prompt).
 Upon termination, you can then choose to use the default for the internal webserver of the application 
 or adapt these to your specific setup.
 ...
 For automated, non-interactive installation, you can pass a YAML or a JSON config file 
 that contains all necessary informations, see details at [TODO link to YAML/JSON install docu].
