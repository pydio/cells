...
import (
	...
	orm "github.com/pydio/cells/common/proto/options"
)
...
func GenerateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	filename := file.GeneratedFilenamePrefix + ".cells.pb.go"
	...
	for _, message := range f.allMessages {
		m, ok := proto.GetExtension(message.Desc.Options(), orm.E_OrmPolicy).(*orm.ORMMessagePolicy)
		if !ok {
			continue
		}
		if m == nil {
...
func fieldGORMTagValue(field *protogen.Field) string {
	v := proto.GetExtension(field.Desc.Options(), orm.E_Field)
	if v == nil {
		return "-:all"
	}

	opts, ok := v.(*orm.ORMFieldOptions)
	if !ok {
		return "-:all"
	}
